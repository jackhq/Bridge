(function() {
  var Bridge, exports;
  Bridge = (function() {
    function Bridge(resource) {
      this.resource = resource;
    }
    Bridge.prototype.all = function(cb) {
      $.getJSON("/" + this.resource).success(function(data, txtStatus) {
        if (data["errors"]) {
          return cb(data["errors"], data);
        } else {
          return cb(null, data);
        }
      }).error(function(data, txtStatus) {
        return cb({
          message: "Server returned and error status " + txtStatus
        }, data);
      });
      return true;
    };
    Bridge.prototype.get = function(id, cb) {
      $.getJSON("/" + this.resource + "/" + id).success(function(data, txtStatus) {
        if (data["errors"]) {
          return cb(data["errors"], data);
        } else {
          return cb(null, data);
        }
      }).error(function(data, txtStatus) {
        return cb({
          message: "Server returned and error status " + txtStatus
        }, data);
      });
      return true;
    };
    Bridge.prototype.create = function(data) {
      return console.log('Create one resource');
    };
    Bridge.prototype.update = function(data) {
      return console.log('Update one resource');
    };
    Bridge.prototype["delete"] = function(id) {
      return console.log('Delete one resource');
    };
    Bridge.prototype.search = function(criteria) {
      return console.log('Criteria');
    };
    Bridge.prototype.valid = function() {
      if (this.resource) {
        return true;
      } else {
        return false;
      }
    };
    return Bridge;
  })();
  if (window) {
    exports = window;
  }
  exports.Bridge = Bridge;
}).call(this);
